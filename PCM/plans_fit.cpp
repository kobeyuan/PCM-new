...
#include <sstream>
 ...
 void PlanClassifier::run()
 {
	...
	//usingPCL();
 	...
 	Sample smpFrame ;
 	SampleManipulation::SubSampleSolution solv( oriFrame, smpFrame,m_reso);
 	...
 	samplePlanModels(smpFrame,smpPlans,m_models);
 
 	vector<IndexType> labels;
 	labels.resize(smpFrame.num_vertices(),0);
 	J_LinkageAdapter_Matlab<PointType,PlanModel,Point2PlanDisFunc> algo_adaptor(smpPoints,smpPlans,labels,Point2PlanDisFunc(0.5),m_tradeOff);
...
//------------------------------
 ...
 void PlanClassifier::setParem(IndexType _activeF, IndexType _res, IndexType _models, ScalarType _thres)
 {
