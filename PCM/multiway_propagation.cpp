...
void DualwayPropagation:: init_labeles_graph_hier(ScalarType distThr)
{
	...
	Loggger<<"  Begin initialize graphs.\n";
...
void DualwayPropagation::split_twoAjacent_graph_next(IndexType srFrame, IndexType tgFrame)
{
	...
	Loggger<<srFrame<<"帧的第"<<gLevel<<"层边界分割"<<tgFrame<<"的"<<tgGraSize - 1 <<"层"<<endl;
	...
	for (EdgeIterator eit = ei.first; eit != ei.second; ++eit)
	{
		...
		{
			Loggger<<"边界点太靠近,不需要分裂.\n";
			continue;
		}

		if (ration < 0.2)
		{
			Loggger<<"Unmark点比值太大,暂时不分裂.\n";
			continue;
		}
...
void DualwayPropagation::split_twoAjacent_graph_prev(IndexType srFrame, IndexType tgFrame)
{
	...
	for (EdgeIterator eit = ei.first; eit != ei.second; ++eit)
	{
		...
		Loggger<<tgFrame<<"帧的边起点"<<ep.start_<<"与终点"<<ep.end_<<"开始分裂"<<endl;
...
// 	}
	...
	for (auto fIter = hier_componets_.begin(); fIter != hier_componets_.end(); ++ fIter)
	{
		...
		Loggger<<"  第"<<fIter->first<<"帧共分割出"<<labeSize<<"个块.\n";
	}
...
void DualwayPropagation::buildMainPatchMatching()
{
	...
	Loggger<<" Using Cosegmentation Labels to build Patches Correspondences.\n";
...
void DualwayPropagation::mergeSingleTinyPatches(IndexType vSize)
{
	...
	Loggger<<"After merge tiny patches.\n";
...
{
	Loggger<<"  Begin initialize point cloud graphs.\n";
   ...
   Loggger<<"  End initialize point cloud graphs.\n";
}
...
void DualwayPropagation::calculateSimilar2Componet()
{
	...
	{
		Loggger<<"read error current path.\n";
		return;
	}
...
{
	Loggger<<"Start paopa label to ori.\n";
	...
	Loggger<<"End propagation label to ori .\n";
}