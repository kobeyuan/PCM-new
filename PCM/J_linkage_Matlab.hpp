...
public:
	...
	void calc_lamda()
	{
		...
		Loggger<<"Lambda:"<<p2l_dist_func_.lamda<<endl;
		delete [] d;
	}
	///
	...
	void compute()
	{
		...
		Loggger<< "ps init elapse time: "<<(size_t)((clock()-old_time)/CLOCKS_PER_SEC)<<"s"<<std::endl;
		Loggger<<"open Matlab Engine.\n";
		old_time = clock();
		Engine*	ep;
		if (! (ep = engOpen(NULL)) )
		{
		...
		Loggger<< "ps merge elapse time: "<<(size_t)((clock()-old_time)/CLOCKS_PER_SEC)<<"s"<<std::endl;
	}
