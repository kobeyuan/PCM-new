...
void GCop::run()
{
	...
	if (isrefine)
	{
		Loggger<<"Start refine single frame.\n";

		refineSegm();

		Loggger<<"End refine single frame.\n";
	}else
	{
		Loggger<<"Start Split & Coseg & Merge Process!\n";
		...
		Loggger<<"End Split Process!\n";

		//cosegProcessing(dp_solver);

		//Loggger<<"End  Coseg Process!\n";

		//mergeProcess(dp_solver);

		//Loggger<<"End Merge Process!\n";
	}
...
void GCop::setNeighbor()
{
	...
	//Loggger<<"sum = "<<stats.col(0).sum()<<endl;
	//Loggger<<"sum = "<<stats.col(1).sum()<<endl;

	//mean_two(0,0)  = stats.col(0).sum()/all_dist.size();
	//mean_two(0,1) = stats.col(1).sum()/all_dist.size();

	//Loggger<<"mean(0,0) = "<<mean_two(0,0)<<endl;
	//Loggger<<"mean(0,1) = "<<mean_two(0,1)<<endl;
	...
	//Loggger<<"sig convarance = "<<endl;
	//Loggger<<sig_two<<endl;
...
void GCop::setDataItem(MatrixXX&totError,std::unordered_map<IndexType,set<IndexType> >& edgePoints )
{
	...
	for (IndexType i = 0; i < nSamples; i ++)
	{
		...
		{
			Loggger<<" find near edge wired????"<<endl;
		}
		delete [] dist2Edge;
...
//
	...
	for (auto iter = gcNode->node_vec.begin(); iter!= gcNode->node_vec.end(); iter ++,itNum++)
	{
		for (IndexType labelId = 0; labelId < numLabes; labelId ++)
		{
			...
			//Loggger<<"label = "<<labelId<< "val = "<<val<<endl;
...
// 	{
// 		Loggger<<samples.row(i)<<endl;
// 		Loggger<<labels(i,0)<<endl;
// 	}
// 
// 
// 	Loggger<<"mean"<<endl;
...
// 	curF.neighbours(node->index,k,neighbours,dist);
	...
	{
		Loggger<<"error ! no correspondence happend.\n";
	}

	//Loggger<<mapId.size()<<endl;
...
// 	Loggger<<"first row of fMat"<<fMat.row(0)<<endl;
// 	Loggger<<"first row of sMat"<<sMat.row(0)<<endl;
...
// 	coffPi = sqrt(coffPi);
   ...
   //Loggger<<"determinant = "<<coffCov<<endl;
...
//    Loggger<<covInve<<endl;
// 
//    Loggger<<"res = "<<endl;
//    Loggger<<cov * covInve<<endl;
// 
//    Loggger<<"coff = "<<coffCov<<endl;

   MatrixXX temp = (sValue - mean_) * covInve * (sValue - mean_).transpose();
   ScalarType mo = temp(0,0);
   ScalarType pro = exp(-mo *(0.5));

//    Loggger<<"prob = "<<pro<<endl;
    //Loggger<<"last = "<<pro/(coffCov * coffPi)<<endl;
...
void GCop::diff_using_bfs( std:: vector<IndexType>& labels,std::vector<IndexType>& centerVtxId,IndexType centerFrame )
{
	...
	Loggger<<"max label before post:"<<new_label + 1<<endl;
	...
	Loggger<<"max label after post:"<<m_nLabels<<endl;
}
...
void GCop::ransacAllAlgorithm(vector<GraphCutNode*>& oriData)
{
	...
	for ( int i = 0; i < nCluster; i ++)
	{
		...
		Loggger<<"tanslation = "<<endl;
		Loggger<<miu.transpose()<<endl;
		Loggger<<"rotmate = "<<endl;
		Loggger<<sigma<<endl;
...
// 		Loggger<<"label = "<<i<<endl;
// 		Loggger<<"tanslation = "<<endl;
// 		Loggger<<trans.transpose()<<endl;
// 		Loggger<<"rotmate = "<<endl;
// 		Loggger<<rotmate<<endl;
// 		Loggger<<"label sr_mean = "<<endl;
// 		Loggger<<sMean.transpose()<<endl;
// 		Loggger<<"label tg_mean = "<<endl;
// 		Loggger<<tMean.transpose()<<endl;
...
void GCop::dist2Transformation(Matrix3X& srCoor,MatrixXX& miu,Matrix33& sigma,set<IndexType>&inliers,ScalarType noiseR,set<IndexType>& graphId,Matrix3X& tCoor)
{
	...
	//Loggger<<"mark value = "<<markDist<<endl;
...
// 旭东写的cosegmentation 代码
	...
	//Loggger<< "propagation time: "<<(size_t)((clock()-old_time)/CLOCKS_PER_SEC)<<"s"<<std::endl;
