...
void GraphMatch::calculateSimilar2Frame()
{
	...
	for(; CFvertex_biter != CFvertex_eiter ; ++CFvertex_biter)
	{
		...
		{
			Loggger<<"Merge this patches to match label"<<vtx_id<<endl;
		...
		Loggger<<srFrame<<"帧的块"<<vtx_id<<"与帧"<<tgFrame<<"的块"<<corLabelId<<"对应.\n";
...
ScalarType GraphMatch::distBetween2Frame4Items(const set<IndexType>& sPatches, const set<IndexType>& tPatches, IndexType srLevel, IndexType tgLevel)
{
	...
	Loggger<<" Source combine patches.\n";
	...
	Loggger<<" Target combine patches.\n";
	...
	Loggger<<" Two condition type distances.\n";
	Loggger<<" corValue = "<<corValue<<endl;
	Loggger<<" scaleShapeValue = "<<scaleShapeValue<<endl;
	//Loggger<<" deformationvalue = "<<deformationValue<<endl;

	if ( scaleShapeValue > 0.2 || corValue < 0.6 || toComVtx.size() < 5 || backComVtx.size() < 5)
	{
		Loggger<<" The distance is INF.\n";
...
// 	--depth_;
	...
	if (!test.second)
	{
		Loggger<<firstId<<" 和"<<secondId<<"不相连.\n";
	...
	Loggger<<" label bucket 大小："<< chierlabucket.size()<<std::endl;
								   ...
								   vector<HLabel*>& old_label_bucket, set<IndexType>& patches ,IndexType& new_label,map<IndexType,IndexType>& LabelIdIndex )
{
	...
	for(; vtxbitr != vtxeitr; ++ vtxbitr)
	{
		...
		//Loggger<<"label 号： "<< cLable.label_id <<"   点的个数 :"<< clabelSize<<std::endl;
...
#define processAndCreateNewGraph_test 0
	...
	for( ; vtxbitr != vtxeitr ; ++vtxbitr)
	{
		...
		}else if( *vtxbitr >_targetVd)
		{
			...
			{
				//Loggger<<"vtxbucket->first"<<vtxbckbitr->first<<"*vtxbitr"<<(*vtxbitr)-1<<std::endl;
				labelofvtx_[ vtxbckbitr->first] = (*vtxbitr) -1;
			}
			//Loggger<<"labelvec_ size"<<_labelvec_.size()<<std::endl;
			newLabelvec.push_back( _labelvec_[ *vtxbitr]);
		...
		if (components_.find(frame_id + 1) != components_.end() )
		{
			...
			for (auto lIter = labelBucket.begin(); lIter != labelBucket.end(); ++lIter, ++ lId )
			{
				...
				if(!isTrav[flKey])
				{
					...
					//Loggger<<" Frame"<<frame_id<<" have "<<vtxNum<<" points, and the Label Id is "<<labeId<<endl;
					if (nextLabelBucket != NULL)
					{
						while (nextLabelBucket != NULL)
						{
							...
							fprintf(outfile,"Frame Id %d have %d  points ,The label Id is %d\n", nFId, nVtxNum, nLId);
							//Loggger<<" Correctness Next patch information is: "<<endl;
							//Loggger<<" Frame id is "<<nFId<<" label Id is "<<nLId<<" and have "<<nVtxNum<<"points"<<endl;
					...
					}else
					{
						...
						//Loggger<<" This patch has not next correspondence!!!!!!.\n";
					}

				}else
				{
					fprintf(outfile," This patch was trav befor!.\n");
					//Loggger<<" This patch was trav befor!.\n";
				}
...
void GraphMatch::mergePatchesAfterCoSeg(IndexType srLevel, IndexType tgLevel,set<IndexType>& srBestSet, set<IndexType>& tgBestSet)
{
	...
	}else
	{
		...
		if (srBestSet.size() > 1)
		{
			Loggger<<" Start Merge source Graph  "<<srFrame<<endl;

			mergeSrPatches(srLevel,srBestSet);

			Loggger<<" End Merge source Graph.\n";
		}
	}
	...
	if (tgBestSet.size() > 1)
	{
		Loggger<<" Start Merge target Graph"<<tgFrame<<endl;

	    mergeTgPatches(tgLevel,tgBestSet);

		Loggger<<" End Merge target Graph.\n";
	}
