...
struct KFEdgeProperty;
struct KFVertexProperty;
...
typedef boost::adjacency_list<boost::listS, boost::vecS, boost::directedS, KFVertexProperty, KFEdgeProperty> VideoGraph;//视频对应的graph
...
//for key frame selection part

typedef boost::graph_traits<VideoGraph>::vertex_descriptor   vdoVertexDescriptor;
// 边描述符
typedef boost::graph_traits<VideoGraph>::edge_descriptor     vdoEdgeDescriptor;
// 下面是邻接链表的一些遍历器
typedef boost::graph_traits<VideoGraph>::vertex_iterator   vdoVertexIterator;
typedef boost::graph_traits<VideoGraph>::edge_iterator   vdoEdgeIterator;
typedef boost::graph_traits<VideoGraph>::adjacency_iterator   vdoAdjacencyIterator;
typedef boost::graph_traits<VideoGraph>::out_edge_iterator   vdoOutEdgeIterator;
...
struct KFVertexProperty 
{
	...
	KFVertexProperty(IndexType fid, IndexType eid)
	{
		framesIdx = fid;
		eventIdx = eid;
	}
...
struct KFEdgeProperty
{
	IndexType eIndex;//edge order
	IndexType start_;//frame idx
	IndexType end_;//frame idx
	ScalarType cost;
	bool isInner; // connect the nodes from the same event?
	...
	KFEdgeProperty(IndexType eId, IndexType _std, IndexType _end, 
		           ScalarType _cost, bool _isIn)
	{
		eIndex = eId;
		start_ = _std;
		end_ = _end;
	...
	bool operator<(const KFEdgeProperty& first) const //后面必须要const,set插入原始必须要有<运算符.
	{
		return this->cost < first.cost;
	}
};

//end for frame selection
