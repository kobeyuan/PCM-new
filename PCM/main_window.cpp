...
//	setWindowState(Qt::WindowMaximized);
	...
	if(m_linkageUi || m_graphCutUi || m_planFitUi || m_paint || m_stopMotionR/*||m_stopMotion*/)
	{
		m_linkageUi  = NULL;
		m_graphCutUi = NULL;
		m_planFitUi = NULL;
...
void main_window::createAlgorithmAction()
{
	...
	connect(ui.actionStop_Motion, SIGNAL(triggered() ),this, SLOT(doStopMotion() ) );
}
...
void main_window::createFileMenuAction()
{
	...
	//connect(ui.actionImportVideoFiles, SIGNAL(triggered() ), this, SLOT(openVideoFiles() ) );
	connect(ui.actionImportVideoFiles, SIGNAL(triggered()), this, SLOT(openVideo4Selection()));

	connect(ui.actionInportImageSeqs, SIGNAL(triggered() ), this, SLOT(openImageSequences() ) );

	connect(ui.actionKeyFrameCap, SIGNAL(triggered()), this, SLOT(openCamera()));
}
...
void main_window::doStopMotion()
{
	//open a widget for stop motion
	//m_stopMotion = new StopMotionUI();
	//m_stopMotion->setWindowTitle("Stop Motion");
	//m_stopMotion->initConnect();
	//m_stopMotion->m_isVideo = m_isReadVideo;
	//m_stopMotion->initBaseInfo();
	//m_stopMotion->show();

	////for texture
	//m_canvas = new CanvasUI();
	//m_stopMotion->setCanva(m_canvas);
	//m_canvas->setWindowTitle("texture interpolation display");
	//m_canvas->show();
	//main_canvas_->updateGL();

	//new UI 11-29
	m_stopMotionR = new StopMotionRUI();
	m_stopMotionR->setWindowTitle("Stop Motion");
	m_stopMotionR->initConnect();
	m_stopMotionR->m_isVideo = m_isReadVideo;
	m_stopMotionR->initBaseInfo();
	m_stopMotionR->showVideoButtons();
	...
	{
		m_canvas = new CanvasUI();
		m_stopMotionR->setCanva(m_canvas);
		m_canvas->setWindowTitle("texture interpolation display");
		m_canvas->show();
		main_canvas_->updateGL();
	}
...
//old version
bool main_window::openVideoFiles()
{
// 	QString dir = QFileDialog::getExistingDirectory(this,tr("Import videos files"),".");
// 	if (dir.isEmpty())
// 		return false;
//F:\Projects\STOP MOTION\Videos - sample\0827
	//QString cur_dir="D:\\project-cityu\\Projects\\Projects\\STOP MOTION\\Videos-sample\\0904-videos";
  QString cur_dir = "F:\\Projects\\STOP MOTION\\Videos-sample\\0827";
	QString fileName=QFileDialog::getOpenFileName(this,"Open Video Files", cur_dir,
		tr("Vedio (*.avi *.wma *.mpeg *.wmv)")
		);
	...
	if (fileName.size() != 0)
	{
	   //imgFileName = (fileInfo.baseName() +".jpg").toStdString();
	   imgFileName = (fileInfo.baseName() + ".jpg").toUtf8().data();
	}

	//open image file location
// 	QString imgFileName = QFileDialog::getOpenFileName(this,"Open Video Files", cur_dir,
// 		tr("Image (*.png *.jpg *.mpeg)")
// 		);
...
// 	}
	...
	string srtFileName = fileName.toStdString();


	const char* fName = srtFileName.c_str();

	SingleFrameSet::get_instance().getTotVideo().open(fName);

	cv::VideoCapture tempV = SingleFrameSet::get_instance().getTotVideo();
	...
	// each frame as data structure

	IndexType totFmeSize = tempV.get(CV_CAP_PROP_FRAME_COUNT);

	Mat tempMat;

	for (LongType i = 0; i < totFmeSize; ++ i)
	{
		SingleFrame* curF = new SingleFrame();
		curF->frameIdx = i;

	    //tempV.set(CV_CAP_PROP_POS_FRAMES, i);
		//tempV>>tempMat;
		//curF->setBoby(&tempMat);

		SingleFrameSet::get_instance().push_back(curF);
	}

	m_isReadVideo = true;

	//QMessageBox::information(this, "Information", "Read a video!",QMessageBox::Ok);
	

	//open pair selection platform
	...
	m_stopMotionR->m_videofileName.assign(imgFileName);

	m_stopMotionR->setGeometry(200,200,1255,825);
	//m_stopMotionR->setBaseSize(1255,800);

	m_stopMotionR->show();

	m_stopMotionR->setFirstStepVideo();

	return true;

}

bool main_window::openVideo4Selection()
{
	//read the video
	QString cur_dir = "F:\\Projects\\STOP MOTION\\Videos-sample\\0827";
	QString fileName = QFileDialog::getOpenFileName(this, "Open Video Files", cur_dir,
	...
	if (fileName.size() != 0)
	{
		imgFileName = (fileInfo.baseName() + ".jpg").toUtf8().constData();
	}
	...
	//open the ui of video show
	m_autoSelection = new DLGAutoSelection();
	m_autoSelection->initConnect();
	m_autoSelection->initBaseInfo();
	m_autoSelection->show();
	//a new UI for image completion process
	//m_stopMotionC = new StopMotionASIG();
	//m_stopMotionC->setWindowTitle("Keyframes selection");
	//m_stopMotionC->initConnect();
	//m_stopMotionC->m_isVideo = m_isReadVideo;

	//m_stopMotionC->atui = *m_autoSelection;

	//m_stopMotionC->atui.setupUi((QDialog*)m_autoSelection);
	//m_stopMotionC->show();

	//m_stopMotionC->atui.show();
	//initialize the control class dlg_stopmotionAsig.h [StopMotionASIG, do not show]


	return  true;
}

bool main_window::openImageSequences()
{
	//QString cur_dir="D:\\project-cityu\\Projects\\Projects\\STOP MOTION\\Hands removal";
	//QString cur_dir="C:\\Users\\YuanQian\\Google Drive\\From dropbox 1015-stopmotion\\23-week-psmooth-examples\\comparison auto and hand\\phone-rotation\\hand-selection";
	//QString cur_dir="D:\\project-cityu\\Projects\\Projects\\STOP MOTION\\Hands removal\\01-25-cub-l6";//cub-large-disparity
	
	//QString cur_dir="D:\\project-cityu\\Projects\\Projects\\STOP MOTION\\Hands removal\\1006-examples\\plane-down";
	

	//QString cur_dir="D:\\project-cityu\\Projects\\Projects\\STOP MOTION\\Hands removal\\sig-example\\selected frames";
    //QString cur_dir="C:\\Users\\YuanQian\\Dropbox\\Stop-motion\\completed examples\\sig-examples\\0107-captured";
	//current project dir
	...
	for (IndexType file_idx = 0; file_idx < file_list.size(); file_idx++)
	{
		...
		if (file_info.suffix() == "bmp" || file_info.suffix() == "BMP")
		{
			file_type = FileIO::BMP;
		}
		else if(file_info.suffix() == "jpg" || file_info.suffix() == "JPG")
		{
			file_type = FileIO::JPG;
		}
		else if(file_info.suffix() == "jpeg" || file_info.suffix() == "JPEG")
		{
             file_type = FileIO::JPEG;
		}else
		{
			continue;
		}
...
// 
// 		string file_name = file_info.baseName().toStdString();

		string file_path = file_info.filePath().toUtf8().constData();

		string file_name = file_info.baseName().toUtf8().constData();

		Mat mtemp = imread(file_path);

		Mat temp(mtemp.rows,mtemp.cols,mtemp.type() );

		if (! mtemp.empty() )
		{
			cvtColor(mtemp, temp, CV_BGR2RGB);

// 			QImage* tImg = new QImage ((const unsigned char*)(temp.data), temp.cols, temp.rows, 
// 				                        temp.step, QImage::Format_RGB888); 
// 			SingleFrame* curF = new SingleFrame(tImg);

			SingleFrame* curF = new SingleFrame;

			curF->setBoby(&temp); 

			curF->frameIdx = file_idx;

			if (!file_name.compare(background))
			{
			   SingleFrameSet::get_instance().setBackGround(curF);
			}else
			{
			   SingleFrameSet::get_instance().push_back(curF);
			}
	...
	//QMessageBox::information(this, "Information", "Read an image sequence!",QMessageBox::Ok);
...
// 	m_stopMotionR->showVideoButtons();
// 
// 	m_stopMotionR->setBaseSize(1488,800);
// 	m_stopMotionR->show();
//end for last version
	...
	//a new UI for image completion process
	m_stopMotionC = new StopMotionASIG();
	m_stopMotionC->setWindowTitle("Stop Motion");
	m_stopMotionC->initConnect();
	m_stopMotionC->m_isVideo = m_isReadVideo;
	m_stopMotionC->initBaseInfo();//initialize the based variance
	m_stopMotionC->showButtonsOne();

	m_stopMotionC->setBaseSize(1488,800);
	m_stopMotionC->show();
	...
	{
		m_canvas = new CanvasUI();
		m_stopMotionC->setCanva(m_canvas);
		m_canvas->setWindowTitle("texture interpolation display");
		m_canvas->show();
		main_canvas_->updateGL();
	}
	...
	m_stopMotionC->initImaLab();
	m_stopMotionC->drawKeyPair(0);
	m_stopMotionC->showSeqinList();

	return true;
}

bool main_window::openCamera()
{
	m_keyFrameCap = new KeyFrameCapture();
	m_keyFrameCap->setWindowTitle("Key Pairs Capture");
	m_keyFrameCap->show();

	m_keyFrameCap->readCameraData();
	//m_keyFrameCap->openLeftCamera();
	//m_keyFrameCap->openRightCamera();
