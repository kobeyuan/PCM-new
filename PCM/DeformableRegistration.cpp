...
{
	m_neigNum = 60;
	//Loggger<<"Local ICP using neighbor K ="<<m_neigNum<<endl;
}
...
// 	Loggger<<"befor ICP source.\n";
// 	Loggger<<neigSrCoor<<endl;
// 
// 	Loggger<<"befor ICP target.\n";
// 	Loggger<<neigTgCoor<<endl;
...
//  	 	     			}
 ...
 // Loggger<<"iter number = "<<iterMax<<endl;
// 	Tracer& tracer = Tracer::get_instance();
// 	tracer.clear_records();
// 	tracer.add_percentCor(0,1,res_cor,1,num_vtx);
...
// 	op.close();
//	Loggger<<"out of distortion number = "<<recordDistortionNum05<<endl;
}
...
void DeformableRegistration::calculateMapFunction(Matrix3X& srGraphCoor,Matrix3X& tgGraphCoor,IndexType axisIndex, Matrix3X & res_mapFunc)
{
	...
	//Loggger<<"Left_mat.\n";
	//Loggger<<Left_mat<<endl;//have infinite element
	  // if (op.is_open())
 		//{  
			//op<<Left_mat<<endl;
 		//	//op.close();
...
void DeformableRegistration::sloveLinearSysByLU(MatrixXX & LeftMat,MatrixXX & RightMat,Matrix3X& res_mapFunc,IndexType axisIndex)
{
	...
	//Loggger<<L<<endl;
	//Loggger<<"L inverse\n";
	//Loggger<<L.inverse()<<endl;

	//Loggger<<"upper = \n";
	//Loggger<<U<<endl;
	//Loggger<<"U inverse\n";
	//Loggger<<U.inverse()<<endl;
	...
	for (IndexType v_it = 0; v_it < sr_num_vtx; v_it++)
	{
		...
		else
		{
			...
			for (IndexType axisIndex = 0; axisIndex < 3; axisIndex++)
			{
				...
				Loggger<<"functions coff.\n";
				Loggger<<mapFunctionCoff.row(axisIndex)<<endl;
				Loggger<<"coor"<<endl;
				Loggger<<rbfCoff.transpose()<<endl;
							                    ...
							                    IndexType pointNum,vector<bool>& isSample)
{
	...
	for(IndexType randIter = 0; randIter < pointNum; randIter ++)
	{
		...
		Loggger<<"rand number = "<<sampleValue<<endl;
...
{
 	Loggger << "Begin Calculate trajectory featureMat.\n";
	...
	//std::fstream op("E:\\DisIndexDistui.txt",std::ios::app);
  	...
  	for (IndexType frameIndex = 1; frameIndex < num_sample;frameIndex++)
  	{
		...
		Loggger<<"frame "<<frameIndex -1<<"Large dist Number = "<<distoredLargeSet.size()<<endl;
		Loggger<<"frame id = "<<frameIndex<<endl;
		Loggger<<"glb correspondences size = "<<corNum<<".\n";
		Loggger<<" produce new trajectory number = "<<tgGraph.num_vertices() - corNum<<".\n";
...
// 	}
	...
	Loggger << "End Calculate trajectory featureMat.\n";
}
...
{
	Loggger << "Begin Calculate  Life - spans trajectory .\n";
	...
	for (IndexType frameIndex = 1; frameIndex < num_sample;frameIndex++)
	{
		...
		Loggger<<frameIndex -1 <<" frame begin is end, never stay this world  = " <<proFaildTraj<<endl;
		validEnd.clear();
		validEnd = nextValidEnd;

		//Loggger<<"before clear traj size = "<<willGoOnTraj.size()<<"should size  = 0.\n";
		willGoOnTraj.clear();
	...
	Loggger<<"End calculate traj-spans.\n";
}
...
{
	Loggger<<"start Calculate  down sample life-span Traj.\n";
	...
	for ( int i = 0; i < end_idx - start_idx; i++ )
	{
		...
		Loggger<<i <<" frame begin is end, never stay this world  = " <<invalidTraj<<endl;
		validEnd.clear();
		validEnd = nextValidEnd;
		nextValidEnd.clear();
	...
	Loggger<<"End Calculate  down sample life-span Traj.\n";
}
...
{
	Loggger<<"start Calculate  down sample life-span Traj from  Center!.\n";
	...
	for ( int i = (smp_idx - start_idx); i < (end_idx - start_idx); i++ )
	{
		...
		Loggger<<i <<"Forward! frame begin is end, never stay this world num size = " <<invalidTraj<<endl;
		validEnd.clear();
		validEnd = nextValidEnd;
		nextValidEnd.clear();
...
// 					IndexType trajEndIndex = (*v_iter).trajNode[trajLife - 1];
		...
		Loggger<<i <<"Backward! frame begin is end, never stay this world num size = " <<invalidTraj<<endl;
		validEnd.clear();
		validEnd = nextValidEnd;
		nextValidEnd.clear();
...
// 	}
	...
	Loggger<<"this length number are =  "<<totsize<<endl;
}
	...
	IndexType vtx_size = globalMap.cols();
 	...
 	for (IndexType tgVtxIter = 0; tgVtxIter < tgVtxNum; tgVtxIter ++)
 	{
		...
		}else
		{
 			...
 			{
 				//Loggger<<"value = "<<iter->second<<endl;//continue 9-29 ---must do it
 			}
		}
...
bool DeformableRegistration::sample3validTraj(vector<PCloudTraj> & totTraj,vector<PCloudTraj>& resTraj,LifeSpan & lifesp)
{
	...
	//
	//Loggger<<"first sample traj is "<<firstTraj<<endl;
	//Loggger<<"second sample traj is "<<secondTraj<<endl;
	//Loggger<<"third sample traj is "<<thirdTraj<<endl;
	//
...
bool DeformableRegistration::sampleValidTraj(vector<PCloudTraj> & totTraj,vector<PCloudTraj>& resTraj,LifeSpan & lifesp,IndexType num)
{
 ...
 void DeformableRegistration::testingcalPmodel()
 {
	 ...
	 //Loggger<<"total traj size = "<<totTraj.size()<<endl;
	 ...
	 //}
	 //Loggger<<"sucess calculate model.\n";

	 ////J_LinkageAdapter<PCloudTraj,PCloudModel,>
	 //Loggger<<"End calculate dis between traj and model.\n";

	 Loggger<<"Begin test TPS.\n";
...
ScalarType DeformableRegistration::calcualteInnerProduct(MatrixXX & oriNorm,MatrixXX& newNorm)
{
 ...
 bool DeformableRegistration::sampleModel(vector<PCloudTraj>& oriTraj,vector<PCloudModel>& smpOriModel,IndexType modelNum)
 {
	   ...
	   PCloudModel resModel;
		...
		// 
		Loggger<<"sample mode size = "<<smpOriModel.size()<<endl;
		return true;
 }
 ...
 {
	 Loggger<<"Start static distance,output .txt file .\n";
	 ...
	 Loggger<<"traj size = "<<trajSize<<endl;
	 Loggger<<"Model size = "<<modelSize<<endl;
 ...
 void DeformableRegistration::produceDreamTraj(vector<PCloudTraj>& resTraj)
 {
	...
	Loggger<<"End calculate dream traj.\n";
 }
...
void DeformableRegistration::calculateFixedLengthTrajWithTracingAlong(vector<PCloudTraj>& trajs, IndexType smp_idx,vector<IndexType>& centerVtxMap,IndexType length, IndexType octreeRes )
	{
		...
		Loggger<<"start Calculate Traj,using concenstive rigid.\n";
...
void DeformableRegistration::smoothSampleLabel(Sample& oriPC,vector<IndexType>& sampleVtxId,vector<IndexType>& label_smp,vector<IndexType>& label_smooth)
{
	...
	Loggger<<"k-neig's k = "<<k<<endl;
	Loggger<<"End  Sample Propagate!"<<endl;
...
void DeformableRegistration::sampleLifeSpanTraj(vector<PCloudTraj> & oriTraj,vector<PCloudTraj>& resTraj,IndexType & cFrame,ScalarType& thresH)
{
   ...
   Loggger<<"traj length morn than  "<<tLen<<endl;
   drawLifespansTraj(resTraj);
}
