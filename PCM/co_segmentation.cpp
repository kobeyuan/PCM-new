...
// 				Loggger<<"Frame = "<<components_[j].frame<<endl;
// 				Loggger<<"Label = "<<components_[j].label<<endl;
...
// 	Loggger<<"  Start calculate simi.\n";
	ScalarType dist = 0.;


// 	Loggger<<" End calculate simi.\n";
	...
	Loggger<<" Start to merge frame"<<srFrame<<"and "<<tgFrame<<endl;
...
void CoSegmentation::co_segment()
{
	...
	{
		set<IndexType>& ss = ii->second;
		Loggger<<"cluster "<<cluster_count++<<endl;
		for ( set<IndexType>::iterator jj=ss.begin(); jj!=ss.end(); jj++ )
		{
			IndexType frame = components_[*jj].frame;
			Loggger<<frame<<"  "<<components_[*jj].label<<endl;
