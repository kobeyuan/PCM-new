function [ output_args ] = drawcurves( values,start,islocal)


x = start:1:(length(values) + start - 1);
set(gca,'xtick',start:10:(length(values) + start - 1))
plot(x, values);
...
%for i = start:1:(length(islocal) + start - 1)  
    %if islocal(i-start + 1) == 1
        ...
        %plot(i, values(i-start + 1),'.','MarkerSize',30);
        %hold on;
    %end
%end
  
title('Foreground Area Distribution');
